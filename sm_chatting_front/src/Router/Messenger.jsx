...
const Messenger = () => {
  ...
  useEffect(() => {
    ...
    client.connect({}, () => {
      ...
      client.send(`/app/first/${chatRoomId[chatRoomId.length - 1]}`, {});

      client.subscribe(`/queue/firstChat/${chatRoomId[chatRoomId.length - 1]}`, function (Message) {
        const newMsg = JSON.parse(Message.body);
        setchatMsg(newMsg);
      });

      client.subscribe(`/queue/addChatToClient/${chatRoomId[chatRoomId.length - 1]}`, function (Message) {
        const newMsg = JSON.parse(Message.body);
